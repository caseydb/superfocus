...
type TabType = "my-rooms" | "quick-join" | "team";
...
const SortableRoomCard: React.FC<SortableRoomCardProps> = ({ 
  ...
  return (
    <div
      ...
      } ${activeTab === "my-rooms" ? "cursor-move" : ""} ${
        isDragging ? "shadow-2xl shadow-[#FFAA00]/20" : ""
      }`}
      {...(activeTab === "my-rooms" ? attributes : {})}
      {...(activeTab === "my-rooms" ? listeners : {})}
    >
      {/* Room Header */}
...
const Rooms: React.FC<RoomsProps> = ({ onClose }) => {
  ...
  const [activeTab, setActiveTab] = useState<TabType>("quick-join");
  ...
  const filteredRooms = useMemo(() => {
    ...
    // For my-rooms tab, show all rooms (we'll filter differently in future)
    if (activeTab === "my-rooms") {
      // Show all rooms for now
      rooms = MOCK_ROOMS;
    }
    ...
    // Sort based on tab
    if (activeTab === "my-rooms") {
      // Sort by custom order, but private rooms first
      ...
      // Sort by active count for quick join
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0a0b0b]/95" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center gap-1 bg-gray-800/50 rounded-full p-1 mb-4">
          <button
            onClick={() => setActiveTab("quick-join")}
        ...
        {/* Search and Create Room - Only show for My Rooms tab */}
        {activeTab === "my-rooms" && (
          ...
          {activeTab === "quick-join" ? (
            ...
            <div className="flex flex-col items-center justify-center h-full p-8">
              <div className="max-w-[480px] w-full text-center space-y-6">
                {/* Icon */}
                <div className="mx-auto w-20 h-20 bg-[#FFAA00]/10 rounded-full flex items-center justify-center">
                  <svg className="w-10 h-10 text-[#FFAA00]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
                  </svg>
                </div>
                ...
                <div>
                  <h3 className="text-2xl font-bold text-gray-200 mb-2">Quick Join a Room</h3>
                  <p className="text-gray-400">
                    Jump into a room with other focused individuals. Perfect for when you want to work alongside others and stay motivated.
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  <button
                    ...
                    className="w-full px-6 py-4 bg-[#FFAA00] text-black font-semibold rounded-lg hover:bg-[#FFB700] transition-colors flex items-center justify-center gap-3"
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <button
                    ...
                    className="w-full px-6 py-4 bg-gray-800 text-gray-300 font-medium rounded-lg hover:bg-gray-700 hover:text-[#FFAA00] transition-colors border border-gray-700 hover:border-[#FFAA00] flex items-center justify-center gap-3"
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="pt-6 border-t border-gray-800">
                  <div className="flex items-center justify-center gap-8 text-sm">
                    <div className="text-center">
                      <p className="text-2xl font-bold text-green-500">{globalActiveRooms}</p>
                      <p className="text-gray-500">Active Rooms</p>
                    </div>
                    <div className="text-center">
                      <p className="text-2xl font-bold text-green-500">{globalActiveUsers.toLocaleString()}</p>
                      <p className="text-gray-500">People Online</p>
                    </div>
                  </div>
                </div>
              </div>
                ...
                {activeId ? (() => {
                  ...
                  return (
                    <div className="pointer-events-none">
                      <div className="p-4 bg-gray-800 rounded-lg border-2 border-[#FFAA00] shadow-2xl shadow-[#FFAA00]/40 opacity-95 flex flex-col min-h-[200px] max-w-[350px]">
                        {/* Room Header */}
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <span className={`text-xs px-2 py-0.5 rounded-full ${
                                ...
                                {activeRoom.type === "private" ? "Vendorsage" : "Public"}
                              </span>
                            </div>
                            {activeRoom.description && (
                              <p className="text-sm text-gray-500 line-clamp-2">{activeRoom.description}</p>
                            )}
                          </div>
                          
                          {/* Quick Stats */}
                          <div className="text-right text-xs text-gray-500">
                            <div className="font-medium text-gray-400">{activeRoom.weeklyStats.totalTime}</div>
                            <div>{activeRoom.weeklyStats.totalTasks} tasks</div>
                            <div className="text-[10px] mt-1">this week</div>
                          </div>
                        </div>
                          ...
                          <div className="flex items-center gap-2 mb-3">
                            <div className="flex -space-x-2">
                              {(() => {
                                const displayItems = [];
                                const maxVisible = 5;
                                const hasOverflow = activeRoom.members.length > maxVisible;
                                const membersToShow = hasOverflow ? 4 : activeRoom.members.length;
                                  ...
                                  displayItems.push(
                                    ...
                                    >
                                      ...
                                      <div className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-gray-900 ${
                                        ...
                                        member.status === "idle" ? "bg-yellow-500" : "bg-gray-600"
                                      }`} />
                                    </div>
                                  );
                                }
                                ...
                                // Add overflow indicator
                                if (hasOverflow) {
                                  const overflowCount = activeRoom.members.length - 4;
                                  displayItems.push(
                                    <div key="overflow" className="relative">
                                      <div className="w-8 h-8 rounded-full border-2 border-gray-900 bg-gray-700 flex items-center justify-center text-xs font-medium text-gray-400">
                                        {overflowCount > 99 ? '99+' : `+${overflowCount}`}
                            ...
                            <span className="text-sm text-gray-400">
                              {activeRoom.activeCount > 0 ? (
                                <span className="text-green-400">{activeRoom.activeCount} active</span>
                              ) : (
                                <span className="text-gray-500">No one active</span>
                              )}
                            </span>
            ...
            <div className="flex flex-col h-full">
              {(() => {
                // Get all Vendorsage (private) rooms
                const teamRooms = MOCK_ROOMS.filter(room => room.type === "private");
                ...
                const sortedMembers = allMembersArray.sort((a, b) => {
                  // Priority order:
                  ...
                  const getStatusPriority = (member: { status: string; task?: string }) => {
                    if (member.status === "online" && member.task && !member.task.includes("Do not disturb")) return 1;
                    if (member.status === "idle") return 2;
                    if (member.status === "online" && member.task && member.task.includes("Do not disturb")) return 3;
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="bg-gray-800/50 rounded-lg p-6 border border-gray-700">
                      ...
                      <div className="grid grid-cols-4 gap-4 mt-6">
                        <div className="text-center">
                          <p className="text-2xl font-bold text-green-400">{totalActiveMembers}</p>
                          <p className="text-sm text-gray-500">Active Now</p>
                        </div>
                        <div className="text-center">
                          <p className="text-2xl font-bold text-green-400">{allMembersArray.length}</p>
                          <p className="text-sm text-gray-500">Total Team Members</p>
                        </div>
                        <div className="text-center">
                          <p className="text-2xl font-bold text-green-400">{formatTotalTime(totalTime)}</p>
